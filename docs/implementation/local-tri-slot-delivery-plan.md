...
仓库已经落地 PostgreSQL 持久化、三版 Alembic 迁移、数据库安全守卫、内容/Canon 导入审核、RenderPlan 编译、真实 Seedream/Seedance SDK 映射、下载与 ffprobe QC、人工审核、恢复、fallback 和三条本地交付。Agent Plan 已完成失败路径烟测，当前生产基线切换为标准 Ark API；文档不记录本机凭据。真实生成仍必须通过 Ark profile 静态检查、数据库和 ffprobe 预检，并由操作者显式确认付费。
...
- Ark 接入支持 `standard` 与 `agent_plan` 两个显式 profile；默认标准
  Ark `/api/v3`，视频模型为 `doubao-seedance-2-0-mini-260615`。
- `httpx` 流式下载媒体。
