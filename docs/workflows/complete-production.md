...
- `SeriesContext`：同一个定稿中性短发儿童、同一只定稿灰白猫、Canon-v1日系二维治愈水彩画风、长期性格和幽默表达。
...
人物Canon固定使用`person:headshot`和`person:fullbody`，猫咪保留三视图。每个Episode在故事板前确定本时段外观签名：Run内已有相同可见外观时复用已批准定妆图，否则创建一次`image:look`单图任务。定妆图审核只检查同一人物、二维画风和本时段服饰是否完整；它不会写回长期Canon。

故事板生成默认只输入三张互不竞争的基准：已批准定妆图作为唯一人物与本集服装来源、与首镜头匹配的猫咪单视图、室内或户外定稿画风图；本集确有必要时最多追加一张用户上传的关键道具图。人物单视图和线条材质图只用于生成定妆图，不在故事板阶段重复输入。故事板审核则使用人物大头照、定妆图、猫咪单视图和定稿画风图作为更完整的比对证据。
...
故事板面板由`ShotPlan`编译：单镜头生成开场、进展和同机位结尾；2镜头生成3张（镜头1主要状态、镜头2进入、镜头2同轴回报）；3镜头生成4张（镜头1、镜头2、镜头3进入、镜头3同轴回报）。每张写入景别、画面方向、主体位置、唯一运镜、动作瞬间、结果和必须继承的身份外观。同一镜头锁定空间轴线、左右关系和相对尺度，明确切镜才允许重新取景。故事板先检查数量、序号、可读取性、9:16比例、统一尺寸和黑边。随后一次语义请求分别检查人物/猫咪身份、身体与尾巴比例、二维画风、动作顺序、同镜头空间、关键道具、服装层和结尾。明确换人、角色复制、严重比例失衡、尾巴连接异常、同镜头无原因位置/尺度跳变、整层服装消失、关键道具变类或结尾未兑现属于硬失败；合理切镜重构、普通背景、轻微表情与眼睛画法只保存为警告。明确失败则整组原子拒绝；低置信转人工；少图、部分下载失败或未批准均不得创建视频任务。
...
人物、猫咪、画风和关键元素Canon只用于Seedream，不与故事板重复传入Seedance。执行Prompt恢复导演执行深度，但只包含三段：
...
每个镜头明确运镜与景别、主体位置、实际执行主体、肢体动作的自然速度和路径、接触对象、可见结果及稳定切点。关键实体只以自然语言表达闭环，不输出数据库ID、生命周期名或重复状态表。`EpisodeScript.soundDesign`保存本集环境底声、关键动作声和结尾声音回报。
