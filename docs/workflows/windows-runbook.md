...
5. 打开场景视觉基准工作台，编辑草稿、选择职责参考、检查 Prompt 预览后按需生成；它不是视频首帧，也不阻断视频。
6. 在顶部点击“生成新候选/重试生成”，提交后可立即关闭工作台；在版本画廊中大图查看、批准、拒绝或重新选择历史视觉基准。
7. 为每个片段选择关闭、只继承造型、完整参考或派生锚点；保存后逐次选择“仅保存”或付费 LLM 分析。
8. 逐项勾选 LLM 字段或正文候选；过期 Revision 必须重新分析。右侧“实际参考图”分别检查锚点和视频输入的真实 `@图片N` 顺序、来源与职责。
9. 选择纯文本、已有图片或生成新锚点；`derive_anchor` 必须先批准锚点。确认 Seedance 费用后任务进入全局任务中心，页面可继续操作。
